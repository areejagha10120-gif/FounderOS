...
/* Requested visual boundaries only -------------------------------------- */
[data-testid="stVerticalBlockBorderWrapper"] {
  background: var(--fos-card) !important;
  border: 1px solid var(--fos-border) !important;
  border-radius: 16px !important;
  box-shadow: 0 8px 24px color-mix(in srgb, var(--text-color) 8%, transparent) !important;
}
...
.fos-purpose {
  border: 1px solid var(--fos-border) !important;
  box-shadow: 0 5px 16px color-mix(in srgb, var(--text-color) 6%, transparent) !important;
}
...
def render_dashboard():
    ...
    with right:
        # Keep the existing position and content, while giving the existing
        # workspace area one clear visual outer boundary.
